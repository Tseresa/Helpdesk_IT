...
@section('title', 'Edit Pengguna - Helpdesk IT')

@section('content')
<div class="max-w-lg">
    <h1 class="text-xl font-semibold mb-1">Edit Pengguna</h1>
    <p class="text-sm text-gray-500 mb-6">{{ $user->full_name }} &middot; {{ $user->email }}</p>
        ...
        <form method="POST" action="{{ route('users.update', $user) }}" class="space-y-4">
                ...
                <div>
                    ...
                    <select id="department_id" name="department_id"
                        ...
                        @foreach ($departments as $department)
                            <option value="{{ $department->department_id }}" @selected(old('department_id', $user->department_id) == $department->department_id)>
            ...
            <div>
                ...
                <input id="password" type="password" name="password"
                       class="w-full rounded-md border border-gray-300 px-3 py-2 text-sm focus:border-brand-700 focus:ring-1 focus:ring-brand-700">
                <p class="text-xs text-gray-400 mt-1">Kosongkan kalau tidak ingin mengubah password.</p>
            </div>
